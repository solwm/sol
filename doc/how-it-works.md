...
2. **Texture upload pass.** For each `SceneElement`, ensure there's a current GL texture for its buffer. SHM buffers go through `glTexSubImage2D` (`upload_shm_texture`); dmabuf buffers go through `EGL_EXT_image_dma_buf_import` (`import_dmabuf_texture`) once per buffer, then we keep the EGLImage and the resulting GL texture in a `HashMap<u64, TextureEntry>` keyed by `buffer_key`.
