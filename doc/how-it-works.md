...
2. **Texture upload pass.** For each `SceneElement`, ensure there's a current GL texture for its buffer. SHM buffers go through `glTexSubImage2D` (`upload_shm_texture`); dmabuf buffers go through `EGL_EXT_image_dma_buf_import` (`import_dmabuf_texture`) once per buffer, then we keep the EGLImage and the resulting GL texture in a `HashMap<u64, TextureEntry>` keyed by `buffer_key`. SHM uploads are gated on a per-buffer `upload_seq` — bumped in the wl_surface.commit handler whenever the client promotes a new buffer, compared against the cached `TextureEntry::uploaded_seq`. Frames where the seq matches skip the upload entirely. The cursor sprite and any layer surface that hasn't been recommitted (waybar between clock ticks, a static wallpaper) coast on the previously-uploaded GPU texture instead of re-uploading every vblank.
